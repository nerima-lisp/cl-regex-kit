  ...
  (let ((regex (compile-regex "a"))
    ...
    (signals
      ...
      (regex-set-matches-into
        regex-set
        (make-array (regex-set-count regex-set) :element-type 'bit :initial-element 0)
        non-matches
        :timeout
        0.0001))
    (dolist (operation
...
(it
  ...
  (let ((b (compile-regex "b"))
    ...
    (expect (cl-regex-kit:full-match-p plus "abbbz" :start 1 :end 4) :to-be-truthy)
...
(it
  ...
  (let ((text
        (make-array 3 :element-type '(unsigned-byte 8) :initial-contents '(65 65 65))))
...
(it
  ...
  (let ((string-regex (compile-regex "x"))
        ...
        (octets (make-array 1 :element-type '(unsigned-byte 8) :initial-contents '(97))))
    (dolist (operation
        (list
          (lambda ()
...
(it
  ...
  (let ((spans nil))
    (do-captures
      ...
      (push
        (list (capture-location-start locations 0) (capture-location-end locations 0))
        spans))
  ...
  (let ((optional-capture-offsets nil))
    (do-captures
      ...
      (push
        (list (capture-location-start locations 1) (capture-location-end locations 1))
...
(it
  ...
  (let ((regex (compile-regex "(?<word>a)")))
    (expect (replace-all regex "a" "$$${word}-$missing-${2}-$") :to-equal "$a---$")
  ...
  (flet ((octets (&rest values)
    ...
    (let ((regex (compile-byte-regex "(?<word>A)")))
      ...
      (expect
        ...
        nil)
      (expect
        (coerce (replace-all regex (octets 65) (octets #x24 #x7b #xff #x7d)) 'list)
        :to-equal
        nil)
      (expect
    ...
    (let ((dotted (compile-byte-regex "(?<a.b>x)")))
      (expect
        (coerce (replace-all dotted (octets #x78) (octets #x24 #x61 #x2e #x62)) 'list)
        :to-equal
        '(46 98))
      (expect
        (coerce
...
(it
  ...
  (expect
    ...
    "right")
  (let ((crlf-text
        (format nil "left~C~Cright~C~Ctail" #\Return #\Newline #\Return #\Newline)))
    (expect
      (match-string
        (scan
    ...
    (flet ((octets (&rest values)
      ...
      (let* ((text (octets #xff #x61 #x0a #x62 #x80))
        ...
        (expect
          ...
          :to-equal
          '(255))
        (signals type-error (replace-first (compile-byte-regex "A") (octets 65) "A"))
        ...
        (let* ((unicode-text (octets #xff #xc3 #xa9 #xfe))
               ...
               (mixed (scan (compile-byte-regex "(?-u:\\xFF)\\p{L}(?-u:\\xFE)") unicode-text)))
            ...
            (destructuring-bind (pattern subject expected) case
              (expect
                (equalp
                  ...
                  (match-string
                    (scan (compile-byte-regex pattern) (ascii-octets subject))
                    (ascii-octets subject)))
                :to-be-truthy)))
          (dolist (case (quote
                (("\\b{start}cat" " cat" "cat")
                  ("cat\\b{end}" "cat " "cat")
            ...
            (destructuring-bind (pattern subject expected) case
              (expect
                ...
                :to-be-truthy)))
          (dolist (case (list
                (list "(?mR)^B" (format nil "A~C~CB" #\Return #\Linefeed) "B")
  ...
  (it
    ...
    (flet ((octets (&rest values)
      ...
      (let* ((text (octets #x20 #xc3 #xa9 #xe2 #x82 #xac #xf0 #x9f #x98 #x80 #x20))
        ...
        (expect (scan (compile-byte-regex "\\b{start}\\p{L}") text :start 2) :to-be nil)
        (dolist (invalid
            (quote
              ((#xc0 #x80)
                (#x80)
                ...
                (#xf4 #x90 #x80 #x80))))
          (expect
            (scan (compile-byte-regex "\\p{L}") (apply #'octets invalid))
            :to-be
            nil))
        (let ((scalar (octets #xc3 #xa9)))
          (expect (scan (compile-byte-regex "\\b") scalar :start 1 :end 1) :to-be nil)
          (expect (scan (compile-byte-regex "\\B") scalar :start 1 :end 1) :to-be nil))))))
...
(it
  ...
  (let ((without-position
    ...
    (expect (regex-syntax-error-reason without-position) :to-equal "unclosed group")
