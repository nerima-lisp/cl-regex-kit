...
(it
  "can disable RE2 octal escapes with Rust-style builder options"
  (dolist (compiler (list #'compile-regex #'compile-byte-regex))
    (signals regex-syntax-error (funcall compiler "\\141" :octal nil))
    (signals regex-syntax-error (funcall compiler "[\\141]" :octal nil)))
  (signals regex-syntax-error (compile-regex-set '("\\141") :octal nil))
  (signals regex-syntax-error (compile-byte-regex-set '("\\141") :octal nil))
  (signals type-error (compile-regex "a" :octal :enabled)))
...
(it
  ...
  (expect (regex-set-match-p (compile-regex-set '("a.b") :literal t) "a.b")
          :to-be-truthy)
...
(it
  ...
  (let ((newline (string #\Newline)))
    ...
    (expect (regex-set-matches
             (compile-regex-set '("(?s:.)" "x") :never-newline t)
             newline)
            :to-equal nil))
  (flet ((octets (&rest values)
    ...
    (expect (regex-set-match-p
             (compile-byte-regex-set '("\\C") :never-newline t)
             (octets #x0a))
            :to-be nil))
...
(it
  ...
  (dolist (case '(("alnum" . #\7)
    ...
    (destructuring-bind (name . character) case
      (expect (match (format nil "[[:~A:]]" name) (string character)) :to-be-truthy)))
...
(it
  ...
  (let ((compiled (cl-regex-kit:byte-regex "ab"))
    ...
    (expect (cl-regex-kit:full-match-p compiled text :start 1 :end 3) :to-be-truthy)
...
(it
  ...
  (dolist (case '((("cat" "dog" "\\d+") "dog42" 0 (1 2))
    ...
    (destructuring-bind (patterns text start expected) case
      (let ((set (compile-regex-set patterns)))
        ...
        (unless (eq (not (null expected)) (regex-set-match-p set text :start start))
  ...
  (let ((set
        (compile-regex-set '("^right$") :multi-line t :crlf t :line-terminator #\;))
        (crlf-text
      ...
      (let ((expected
            (loop for pattern in patterns
                  ...
                  when (scan (compile-regex pattern) text :start start)
                    collect index))
            (actual (regex-set-matches (compile-regex-set patterns) text :start start)))
    ...
    (destructuring-bind (pattern text expected) case
      (let ((actual (match-string (match pattern text) text)))
        (unless (string= actual expected)
          (error "Pattern ~S on ~S: expected ~S, got ~S" pattern text expected actual))))))
