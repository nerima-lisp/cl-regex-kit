      ...
      (multiple-value-bind (marks generation) (advance-pike-vm-closure-workspace workspace (length program))
        ...
        (labels ((enqueue (pc)
          ...
          (dolist (pc seeds)
            (visit pc))
          (cdr head)))))
  (defun pike-vm-boolean-closure (program text position length byte-mode-p seeds &key workspace)
    "Compute an integer-PC epsilon closure for a capture-free boolean match."
    (declare (type simple-vector program))
    (let ((workspace (or workspace (make-pike-vm-closure-workspace (length program))))
          (head (list nil))
          (tail nil))
