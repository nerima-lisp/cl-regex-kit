...
(defun ranges-matcher (ranges)
  (list :ranges ranges))

(defun union-matcher (parts)
  (cond
    ((null parts) (ranges-matcher nil))
...
between-items position."
  (let ((first (peek-token)))
    (when (and
        first
        (member (token-type first) '(:char :hyphen))
        (member (token-value first) '(#\& #\~ #\-)))
      (let ((saved *regex-token-position*)
            (first-character (token-value first)))
        (take-token)
        (let ((second (peek-token)))
          (prog1
            (and
              second
              (member (token-type second) '(:char :hyphen))
              (char= (token-value second) first-character)
...
neither `]` nor another `-` -- probed without consuming the hyphen."
  (let ((saved *regex-token-position*))
    ...
    (prog1
      (let ((next (peek-token)))
        (and next (not (member (token-type next) '(:rbracket :hyphen)))))
      (setf *regex-token-position* saved))))
...
(defun parse-union ()
  (let ((ranges nil)
        ...
        (saw-item-p nil))
    (loop (when (at-end-p)
        (fail "Unclosed character class")) (when (or (peek-type :rbracket) (class-set-operator))
        (return)) (setf saw-item-p t) (cond
...
(defun parse-class ()
  (take-token)
  (let ((negated-p
        (when (and (peek-type :char) (char= (peek-value) #\^))
          (take-token))))
    (when (at-end-p)
      (fail "Unclosed character class"))
    (if (peek-type :rbracket) (progn
        (take-token)
