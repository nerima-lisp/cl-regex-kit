...
(defun hex-digit-p (character) (and character (digit-char-p character 16)))
(defun octal-digit-p (character) (and character (digit-char-p character 8)))
...
sensitive in this grammar."
  (let ((end position) (length (length pattern)))
...
(defun scan-hex-code (pattern position digits)
  (let ((beginning position))
    (multiple-value-bind (hex next) (scan-while pattern position #'hex-digit-p)
      ...
      (values (unicode-scalar-character pattern (parse-integer hex :radix 16) beginning) next))))
...
(defun scan-octal-code (pattern position)
  ...
  (let ((beginning (1- position)))
    (multiple-value-bind (digits next)
        (scan-while pattern position (lambda (c) (and (octal-digit-p c) t)))
...
mode whitespace tolerance inside the braces."
  (let ((escaped (char pattern position)) (next (1+ position)))
    (case escaped
      ...
      (#\x (multiple-value-bind (character after) (scan-hex-code pattern next 2)
             (values character t after)))
      (#\u (multiple-value-bind (character after) (scan-hex-code pattern next 4)
             (values character nil after)))
      (#\U (multiple-value-bind (character after) (scan-hex-code pattern next 8)
             (values character nil after))))))

(defun scan-unicode-property-name (pattern position)
  "POSITION is just past \\p or \\P. Returns (values name negated-p next-position)."
  (if (and (< position (length pattern)) (char= (char pattern position) #\{))
      (let ((body-start (1+ position)))
        (multiple-value-bind (name next) (scan-while pattern body-start (lambda (c) (not (char= c #\}))))
          ...
          (let ((not-equal-position (search "!=" name)) (negated-p nil))
            (cond
              (not-equal-position
               (setf name (format nil "~A=~A" (subseq name 0 not-equal-position)
                                   (subseq name (+ not-equal-position 2)))
                     negated-p t))
              (t (let ((separator (or (position #\= name) (position #\: name))))
                   (when separator
                     (setf name (format nil "~A=~A" (subseq name 0 separator)
                                         (subseq name (1+ separator))))))))
            (unless (unicode-property-name-p name)
              (tokenizer-fail pattern body-start "Unknown or unsupported Unicode property"))
            (values name negated-p (1+ next)))))
      (progn
        (unless (< position (length pattern))
          (tokenizer-fail pattern position "Expected Unicode property name"))
        (let ((name (string (char pattern position))))
          (unless (unicode-property-name-p name)
            (tokenizer-fail pattern position "Unknown or unsupported Unicode property"))
          (values name nil (1+ position))))))
...
unterminated \\Q runs to the end of PATTERN, matching RE2/Rust semantics."
  (let ((end position) (length (length pattern)))
    (loop while (and (< end length)
                      (not (and (char= (char pattern end) #\\)
                                (< (1+ end) length)
                                (char= (char pattern (1+ end)) #\E))))
          do (incf end))
    (values (subseq pattern position end) (if (< end length) (+ end 2) end))))
