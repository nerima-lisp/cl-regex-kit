...
(progn
  ...
  (defun extra-binary-property-ranges (name)
    (let ((canonical
          (or
            (cdr (assoc name +unicode-extra-binary-property-aliases+ :test (function string=)))
            name)))
      (cdr (assoc canonical +unicode-extra-binary-property-ranges+ :test (function string=))))))
