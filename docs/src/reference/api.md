...
```lisp
(replace-first regex text replacement &key (start 0) end timeout) => string
(replace-all regex text replacement &key (start 0) end timeout) => string
(replace-n regex text replacement count &key (start 0) end timeout) => string
```
...
Unbraced names after `$` use only ASCII letters, digits, and `_`; use `${name}`
for capture names containing Unicode characters, `.`, `[`, or `]`.
`replace-n` performs at most `count` non-overlapping replacements, matching
Rust `Regex::replacen`; `count` must be a non-negative integer. For byte
regexes, every replacement function returns an octet vector instead of a
string.
