...
Signalled when an advanced pattern exceeds the bounded AST executor's
configured `:size-limit` or `:nest-limit` during matching. Readers:
`advanced-regex-limit-kind` (`:steps`, `:states`, or `:nest-depth`),
`advanced-regex-limit`, and `advanced-regex-limit-used`. The regular
Thompson NFA/Pike VM path retains its compile-time `:size-limit` behavior.
